...
#' @return `NULL` invisibly. Called for the side effect of creating instrument
...
#'
#' This function exists to tell \code{\link[quantmod]{getSymbols}} where to look
...
#' all Windows machines may use \dQuote{M:/} and all *nix-style (linux, Mac)
#' machines may use \dQuote{/mnt/mktdata/}.
#'
#' The \code{split_method} currently allows either \sQuote{days} or
#' \sQuote{common}, and expects the file or files to be in sub-directories named
...
#' @return No meaningful return value. Called for the side effect of updating
#'   the `quantmod` symbol lookup table for the requested instruments.
#' @seealso
#' \code{\link{getSymbols.FI}},
...
#' @return If `auto.assign = TRUE`, a character vector containing the names of
#'   the objects assigned to `env`. If `auto.assign = FALSE`, an `xts` object
#'   is returned for one requested symbol and a named list is returned for
#'   multiple symbols. Returns `NULL` when no data are found.
#' @seealso
