...
saveInstruments <- function(file_name = "MyInstruments",
                            dir = "",
                            compress = "gzip") {
  ...
  extension <- if (
    tolower(tail(ssfn, 1L)) %in% c("rda", "rdata", "r", "txt")
  ) {
    file_name <- paste(
      ssfn[seq_len(length(ssfn) - 1L)],
      collapse = "."
    )
    tail(ssfn, 1L)
  } else {
  ...
  output_name <- paste0(file_name, ".", extension)

  file.name <- if (is.null(dir) || identical(dir, "")) {
    output_name
  } else {
    file.path(dir, output_name)
  }
  ...
  if (tolower(extension) %in% c("r", "txt")) {
    ...
    writeLines(
      c(
        "# Auto-generated by FinancialInstrument::saveInstruments",
        ""
      ),
      con
    )
    ...
    for (s in ls_instruments()) {
      writeLines(
        "FinancialInstrument::loadInstruments(list(",
        con
      )
